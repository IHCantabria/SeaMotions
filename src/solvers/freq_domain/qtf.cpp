    ...
    for ( int i=0; i<input->heads_np; i++ )
    {
        for ( int j=0; j<mesh_gp->meshes_np; j++ )
        {
            ...
            for ( int k=pot_gp->field_points_cnp[j]/ngpf_1d; k<pot_gp->field_points_cnp[j+1]/ngpf_1d; k++ )
            {
                ...
                std::cout << "nx: " << panel_k->normal_vec[0] << " - int_mod: " << int_mod << " - wl_x: " << qtf_values[idx0] << std::endl;
            }
        }
    }
    ...
    for ( int i=0; i<input->heads_np; i++ )
    {
        for ( int j=0; j<mesh_gp->meshes_np; j++ )
        {
            ...
            for ( int k= vel_gp->field_points_cnp[j]/ngpf; k<vel_gp->field_points_cnp[j+1]/ngpf; k++ )
            {
                ...
                std::cout << "Panel[" << k << "]: " << int_mod << " - fx: " << qtf_bern[idx0] << std::endl;
            }
        }
    }
...
void    calculate_second_order_force_nlin(
                                                Input*          input,
                                                MpiConfig*      mpi_config,
                                                MeshGroup*      mesh_gp,
                                                cuscomplex*     mdrift_rel_we_i,
    ...
    for ( int i=0; i<input->heads_np; i++ )
    {
        for ( int j=0; j<mesh_gp->meshes_np; j++ )
        {
            ...
            for ( int k=mesh_gp->panels_wl_cnp[j]; k<mesh_gp->panels_wl_cnp[j+1]; k++ )
            {
                ...
                int_mod             = mdrift_rel_we_i[idx1] * std::conj( mdrift_rel_we_j[idx1] ) * panel_k->len_wl;
                wl_x                -= 0.25 * input->grav_acc * input->water_density * int_mod * panel_k->normal_vec[0];

                for ( int r=0; r<3; r++ ) std::cout << panel_k->center_wl[r] << " - ";
                std::cout << "nx: " << panel_k->normal_vec[0] << " - int_mod: " << int_mod << " - wl_x: " << wl_x << std::endl;
