 ...
 * @param   Field name where the inconsistency was detected
 */
template<typename T>
inline  void    _print_gen_alpha_inconsistency_msg(
                                                int                 index,
                                                const T&            value,
                                                const std::string   quantity_name
                                            )
{
    std::cerr << "[WARN]: GeneralizedAlpha solver detected inconsistency between "
                 "restricted degrees of freedom and initial boundary conditions kinematics.\n";
    std::cerr << "\t\t-> " << quantity_name << "[ " << index << " ]: " << value << "\n";
}

//--------------------------------------------------------------------
//-- Define Generalized-Alpha solver
//--------------------------------------------------------------------

/**
 * @brief Generalized-Alpha time solver implementation with constant time step.
 *
 * Implements the second-order accurate, unconditionally stable method from:
 ...
 *
 * The algorithm parameters are fully determined by a single user-supplied spectral
 * radius rho_inf ∈ [0, 1] at infinite frequency:
 *   alpha_m = (2*rho_inf - 1) / (rho_inf + 1)
 *   alpha_f =    rho_inf      / (rho_inf + 1)
 *   gamma   = 0.5 - alpha_m + alpha_f
 *   beta    = (1  - alpha_m + alpha_f)^2 / 4
 *
 * rho_inf = 1 → no numerical dissipation (coincides with the average acceleration rule).
 * rho_inf = 0 → maximum high-frequency dissipation.
 *
...
private:
     ...
     *
     * This method pretends to unify the imposition of restrictions for kinematics and
     ...
     * @param   Spectral radius at infinite frequency rho_inf ∈ [0, 1]
...
public:
    ...
    cusfloat            alpha_f         = 0.0;
    cusfloat            alpha_m         = 0.0;
    cusfloat            beta            = 0.0;
    int                 count           = 0;
