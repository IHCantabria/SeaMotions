...
void check_vsl_error( int num )
{
    switch(num) {
        ...
        {
            printf("Error: advanced skipahead initialization is unsupported (code %d).\n",num);
            break;
        }
...
int64_t generate_random_seed( void )
{
    ...
    int64_t time_millis     = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
    int64_t round_value     = 1e4;
    int64_t time_seed       = time_millis%round_value;

    // Generate process seed
    #if defined( WIN32 )
    int64_t pid_seed        = _getpid();
    #elif defined( __unix__ )
    int64_t pid_seed        = getpid();
    #endif

    // Combine seeds to make the generation less
    // specific of the system and the launching time
    int64_t seed            = time_seed + pid_seed;
...
{
    /**
     * @brief Gauss random number generator
     * 
     * @param mean  Distribution mean value
     * @param std   Distribution standard deviation
     * @param seed  Initial seed to generate random numbers
     * @param np    Number of points required
     * @param data  Vector to storage the random numbers generated
     * 
     */
    ////////////////////////
    /***** Check Input*****/
    ////////////////////////
    assert( std > 0.0 && "ERROR. Function: randn - Input parameter std cannot be smaller than 0." );
    ...
    check_vsl_error( errcode );

    //////////////////////
    /***** Call RNG *****/
    //////////////////////
    errcode = vdRngGaussian( VSL_RNG_METHOD_GAUSSIAN_ICDF, stream, np, data, mean, std );
    check_vsl_error( errcode );
...
void randu( double a, double b, int seed, int np, double* data )
{
     ...
     * 
     * @param a     Left distribution bound
     * @param b     Right distribution bound
     * @param seed  Initial seed to generate random numbers
     * @param np    Number of points required
     * @param data  Vector to storage the random numbers generated
     * 
    ...
    /***** Check Input*****/
    ////////////////////////
    assert( b > a && "ERROR. Function: randu - Input parameter b cannot be smaller than a." );
    ...
    /***** Call RNG *****/
    //////////////////////
    errcode = vdRngUniform( VSL_RNG_METHOD_UNIFORM_STD_ACCURATE, stream, np, data, a, b );
    check_vsl_error( errcode );
